...
class AuthController extends Controller
{
    ...
    function resendVerificationMail(Request $request)
    {
        ...
        $user->ApiSendEmailVerificationNotification();
...
function updateUserPhoto(Request $request)
{
    ...
    try {
        ...
        if ($user->getRawOriginal('photo')) {
            $this->discardImage($user->getRawOriginal('photo'), 'profile');
        }

        $user->photo = null;

        if (!empty($request->photo)) {
            // For base64, pass the string directly, NOT $request->file()
            $user->photo = $this->storeImage($request->photo, 'profile');
        }
